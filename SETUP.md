...
아무것도 없는 상태에서 **dev 환경(데스크탑)을 만들고, prod 환경(라즈베리파이 2대)에 배포해
운영하기까지**의 가이드다. 설명 없이 명령만 순서대로 따라가려면 [INSTALL.md](INSTALL.md) 를 본다.

환경은 둘뿐이고 장비로 구분된다 — **데스크탑이 dev, 라즈베리파이가 prod** 다.

| 문서 | 역할 |
|---|---|
| 본 문서 | 환경 구축 → 배포 → 운영. **왜 그렇게 하는지**와 선택지 |
| [INSTALL.md](INSTALL.md) | 이 구성에 올리는 **실제 순서**. 명령만 담긴 복붙용 순서서 |
...
| 장비 | 환경 | 하는 일 | 하지 않는 일 |
|---|---|---|---|
| 데스크탑 PC | **dev** | 코드 수정, 테스트, **빌드**, 배포 실행 | 서비스 상시 운영 |
| 앱 서버 파이 `192.168.45.101` | **prod** | 백엔드·프론트엔드 실행 | **빌드** (받은 이미지만 실행) |
| DB 서버 파이 `192.168.45.102` | **prod** | MySQL 실행, 데이터 보관 | 애플리케이션 실행 |

**라즈베리파이 = prod 다.** 파이에서 도는 것은 언제나 배포 환경이며, 실제 가계부 데이터가
있는 곳이다. 개발과 실험은 데스크탑에서만 한다.
