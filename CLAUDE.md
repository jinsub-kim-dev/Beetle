...
**어느 장비가 어느 환경인지**는 아래로 고정입니다. 이 대응을 벗어나는 구성은 없습니다.

| 환경 | 도는 장비 | 비고 |
|---|---|---|
| **dev** | 데스크탑 PC 의 도커 | 개발·검증만 합니다. 서비스를 상시 운영하지 않습니다 |
| **prod** | **라즈베리파이 2대** (앱 서버 `192.168.45.101` · DB 서버 `192.168.45.102`) | 실제 가계부 데이터가 있는 곳입니다 |

파이는 개발용이 아닙니다. **파이에서 도는 것은 언제나 prod** 이며, 파이에 dev 프로필로
무언가를 띄우는 절차는 문서에 없고 만들지 않습니다. 실험은 데스크탑에서 합니다.

prod 는 한 호스트에 전부 올릴 수도 있고, **서버와 DB 를 나눌 수도** 있습니다. 지금 구성은
나눈 쪽이며, 호스트별 파일을 덧붙입니다.
...
| 브랜치 | 역할 |
|---|---|
| `dev` | 개발이 이뤄지는 곳. 기본 작업 브랜치입니다 |
| `main` | **배포 기준.** 검증이 끝난 상태만 `dev` 에서 머지합니다 |

배포 호스트는 `main` 을 받습니다(`SETUP.md` 2절). 아직 머지되지 않은 변경을 배포해야 하는
경우에만 `dev` 를 받습니다.
...
- `INSTALL.md` — 설치 순서서. 이 구성(파이 2대)의 실제 주소·계정이 들어간 명령 나열입니다.
  **명령만 담고 이유는 담지 않습니다.** 이유는 `SETUP.md` 에 있고, 정책이 바뀌면 양쪽을 함께 고칩니다
- `SETUP.md` — 환경 구축 가이드 (로컬 처음부터, 라즈베리파이 배포, 운영, 모니터링)
