...
2. **상태 관리 분리:**
   ...
   - **필터 상태는 성격에 따라 위치를 나눕니다.**
     ...
     | 거래 목록 필터 (카테고리·결제 수단) | **URL 쿼리 파라미터** | 통계 화면에서 링크로 전달돼야 합니다 |
...
## 4. 핵심 화면 구성 (UI Pages)
- **대시보드 (Dashboard):** 이번 달 요약, 고정비/변동비 비중 도넛 차트, **큰 지출 Top 5**,
  최근 거래 리스트
- **거래 내역 (Transactions):** 월별/조건별 필터, 테이블 뷰, 출금 완료 토글
...
- **통계 및 분석 (Analytics):** 기간별 소비 추이(수입·지출 막대 + 수지 선), 카테고리·카드사별
  점유율, 할부 현황판. **점유율 항목은 거래 목록으로 가는 드릴다운 링크입니다.**
- **설정 (Settings):** 고정비/변동비 카테고리 목록, 결제 수단(카드사/계좌) 목록·등록
