...
| 화면 | 내용 |
|---|---|
| 대시보드 | 이번 달 수입/지출/수지(**전월 대비 병기**), 다음 달 청구 예정액, **평소보다 많이 쓴 항목**, 고정비·변동비 도넛, **큰 지출 Top 5**, 최근 거래 |
| 거래 내역 | 카테고리·결제 수단 필터, **메모 검색**, 건수·합계, 소비일과 청구일을 나란히 보여주는 표, 출금 완료 토글 |
| 거래 등록 | 모든 화면 상단의 `거래 등록` 버튼. 청구일은 서버가 산출하고 결과를 바로 보여준다 |
| 통계·분석 | **전월/작년 같은 달 대비**(요약 + 늘어난·줄어든 지출), 최근 6개월 추이(수입·지출 막대 + 수지 선), 카테고리·카드별 점유율, 할부 현황 |
